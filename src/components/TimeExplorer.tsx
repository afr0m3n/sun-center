...
import { civilTimelineInstant, formatCivilTimelineInstant, getCivilTimelineRangeState, timelineFraction } from '../astronomy/civilTimeline'
...
export function TimeExplorer({ timeline, events, instant, isLive, onInspect, onNow }: TimeExplorerProps) {
  ...
  return <section className="panel time-explorer" aria-labelledby="time-explorer-title">
    <div className="panel-heading">
      ...
      <div className={`mode-badge ${isLive ? 'live' : 'inspect'}`} role="status" aria-live="polite"><span />{isLive ? 'LIVE' : 'INSPECT'}</div>
    </div>
    ...
    <div className="timeline-shell">
      ...
      <input aria-label="Inspect time within selected civil day" aria-valuetext={`${timeline.date} ${formatCivilTimelineInstant(timeline, instant, true)}`} type="range" min={range.min} max={range.max} step={60} value={range.value} onChange={(event) => inspectElapsed(Number(event.target.value))} />
