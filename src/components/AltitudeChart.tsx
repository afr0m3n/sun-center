...
import { civilInstantFromPlotX, useChartViewWidth } from './useCompactChartLayout'
...
export function AltitudeChart({ location, date, samples, events, instant, onInspect }: AltitudeChartProps) {
  ...
  const inspectFromPointer = (event: PointerEvent<SVGSVGElement>) => {
    ...
    onInspect(civilInstantFromPlotX(timeline, local.x, padding.left, plotWidth))
  }
  ...
  const moveDrag = (event: PointerEvent<SVGSVGElement>) => { if (dragPointer === event.pointerId) inspectFromPointer(event) }
  ...
  return <div className="chart-scroll" aria-label={`Solar altitude chart for ${date}`}><svg className="altitude-chart" viewBox={`0 0 ${width} ${height}`} role="button" aria-label="Solar altitude graph; click, drag, or use left and right arrows to inspect" tabIndex={0} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onLostPointerCapture={() => setDragPointer(null)} onKeyDown={inspectFromKeyboard}>
    <title>Solar altitude throughout the selected local day; click or drag to inspect</title><defs><linearGradient id="sun-line" x1="0" x2="1"><stop offset="0" stopColor="var(--series-sunset)"/><stop offset="0.5" stopColor="var(--series-maximum)"/><stop offset="1" stopColor="var(--series-sunset)"/></linearGradient></defs>
    ...
    {dragPointer !== null && <g className="chart-drag-tooltip" pointerEvents="none" transform={`translate(${Math.min(width - 132, Math.max(4, x(instant) + 9))} ${Math.max(4, y(inspectedPosition.altitudeDeg) - 33)})`}><rect width="128" height="27" rx="3"/><text x="7" y="18">{DateTime.fromJSDate(instant, { zone: location.timezone }).toFormat('HH:mm:ss')} · {inspectedPosition.altitudeDeg.toFixed(1)}°</text></g>}
  </svg></div>
}
